...
def get_all_contacts(current_user: dict = Depends(require_admin_or_read_only), cursor=Depends(get_db_cursor)):
    ...
    """
    return contact_service.get_all_contacts(cursor)
...
def sync_full_contact(payload: ContactSyncPayload, background_tasks: BackgroundTasks = BackgroundTasks(), 
    ...
    """
    res = contact_service.sync_full_contact(cursor, payload, current_user)
...
def delete_global_contact(contact_id: str, background_tasks: BackgroundTasks = BackgroundTasks(), 
    ...
    """
    res = contact_service.delete_global_contact(cursor, contact_id, current_user)
    background_tasks.add_task(manager.broadcast, '{"action": "REFRESH_ASSETS"}')
    return res
...
def get_asset_contacts(raw_asset_id: str, current_user: dict = Depends(get_current_user), cursor=Depends(get_db_cursor)):
    ...
    """
    return contact_service.get_asset_contacts(cursor, raw_asset_id)
...
def get_country_contacts(country_id: str, current_user: dict = Depends(get_current_user), cursor=Depends(get_db_cursor)):
    ...
    """
    return contact_service.get_country_contacts(cursor, country_id)