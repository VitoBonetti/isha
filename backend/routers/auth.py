...
def get_current_user(request: Request, api_key: str = Depends(api_key_header), cursor=Depends(get_db_cursor)):
    # METHOD A: API KEY AUTHENTICATION
    if api_key:
        hashed = hash_api_key(api_key)
        cursor.execute("""
                SELECT u.id, u.email, u.name, u.role, u.location_id, u.service_lane_id, ak.is_read_only
                FROM users u
                JOIN api_keys ak ON u.id = ak.user_id
                WHERE ak.hashed_key = %s 
                  AND ak.is_active = TRUE 
                  AND (ak.expires_at IS NULL OR ak.expires_at > CURRENT_TIMESTAMP AT TIME ZONE 'UTC')
            """, (hashed,))
        ...
        is_key_read_only = bool(user[6])

        # Enforce Read-Only API Key restriction centrally
        if is_key_read_only and request.method in ["POST", "PUT", "DELETE", "PATCH"]:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="This API key is restricted to Read-Only access."
            )

        user_dict = {
            "id": user[0], "email": user[1], "name": user[2], "role": user[3],
            "location_id": user[4], "service_lane_id": user[5], "auth_method": "API_KEY",
            "is_key_read_only": is_key_read_only
        }
...
def list_api_keys(global_view: bool = False, current_user: dict = Depends(get_current_user), cursor=Depends(get_db_cursor)):
    if global_view and current_user['role'] == 'admin':
        ...
        cursor.execute("""
            SELECT id, name as key_name, prefix, created_at, expires_at, is_read_only, NULL as owner_name, NULL as owner_email
            FROM api_keys 
            WHERE user_id = %s 
...
def create_api_key(req: ApiKeyCreate, current_user: dict = Depends(get_current_user), cursor=Depends(get_db_cursor)):
    ...
    cursor.execute(
        """INSERT INTO api_keys (id, user_id, name, prefix, hashed_key, created_at, expires_at, is_active, is_read_only) 
           VALUES (%s, %s, %s, %s, %s, CURRENT_TIMESTAMP, %s, TRUE, %s)""",
        (new_id, str(current_user['id']), req.name, prefix, hashed, expires_at, req.is_read_only)
    )
    cursor.connection.commit()

    log_audit_event(
        user_id=str(current_user["id"]), role=current_user["role"], action="CREATE_API_KEY",
        resource_type="USER", resource_id=str(new_id),
        details=f"{current_user['id']} created a {'Read-Only' if req.is_read_only else 'Read/Write'} API Key."
    )

    return {
        "id": new_id, "name": req.name, "prefix": prefix, "raw_key": raw_key,
        "expires_at": expires_at.isoformat(), "is_read_only": req.is_read_only,
...
async def trigger_api_key_alerts(background_tasks: BackgroundTasks, current_user: dict = Depends(require_admin), cursor=Depends(get_db_cursor)):
    """
        Manually forces the API key expiration check to run.
        (The automated daily check is handled by the internal asyncio scheduler).
        """
    ...
    log_audit_event(
        ...
        resource_id="API_KEY_ALERTS",
        details="Admin manually triggered the API key expiration cron job."
    )

    return {"message": "Manual API key expiration check executed successfully."}
...
async def start_daily_api_key_alert_scheduler():
    ...
    while True:
        ...
        target = datetime.combine(now.date(), time(8, 0, 0), tzinfo=timezone.utc)
