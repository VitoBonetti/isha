...
# --- HELPER: ENUM MAPPING ---
# Translates frontend human-readable strings BACK to strict PostgreSQL ENUMs
...
def update_test(test_id: str, t: TestBase, background_tasks: BackgroundTasks,
                current_user: dict = Depends(require_admin), cursor=Depends(get_db_cursor)):
    # Safely translate the frontend status to the DB Enum
    ...
    try:
        with db_cursor_context() as cursor:
            ...
            for asset_id in asset_ids:
                ...
                cursor.execute('INSERT INTO test_assets (test_id, asset_id) VALUES (%s, %s)',
                               (new_test_id, str(asset_id)))
                cursor.execute('UPDATE assets SET is_assigned = TRUE WHERE id = %s', (str(asset_id),))

            cursor.connection.commit()

    except Exception as e:
        print(f"\n[DEBUG] !!! EXCEPTION IN BULK WORKER: {e} !!!\n")
...
def mark_test_unable(test_id: str, background_tasks: BackgroundTasks,
    ...
    # 2. Create the Tombstone (Red Ghost Card) as 'STOPPED'
    cursor.execute('''
    ...
    # 3. Move the assignments to the Tombstone so the capacity is visibly "burned" on the board
    cursor.execute('UPDATE assignments SET test_id = %s WHERE test_id = %s', (tombstone_id, test_id))

    # 4. Clone the asset relationships to the tombstone for historical record
    cursor.execute('SELECT asset_id FROM test_assets WHERE test_id = %s', (test_id,))
    for (asset_id,) in cursor.fetchall():
        cursor.execute('INSERT INTO test_assets (test_id, asset_id) VALUES (%s, %s)', (tombstone_id, str(asset_id)))

    # 5. Send the original test back to the Backlog (NOT_PLANNED)
    cursor.execute("UPDATE tests SET start_week = NULL, start_year = NULL, stages = 'NOT_PLANNED' WHERE id = %s",
                   (test_id,))

    cursor.connection.commit()
    background_tasks.add_task(manager.broadcast, '{"action": "REFRESH_BOARD"}')
    return {"message": "Test marked as Stopped. Capacity burned, original returned to backlog."}
...
def get_test_history(test_id: str, current_user: dict = Depends(get_current_user), cursor=Depends(get_db_cursor)):
    cursor.execute('''
        SELECT th.id, th.action, th.details, th.created_at, u.name as user_name
