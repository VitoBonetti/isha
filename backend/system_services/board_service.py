...
def get_user_provision_internal(db: Session, user_id, year, week_number):
    ...
    # Find the Global location ID for National Holidays
    global_loc = db.query(Locations.id).filter(Locations.name == 'Global').first()
    global_loc_id = global_loc[0] if global_loc else None
    ...
    for e in events:
        ...
        event_dates = [(e.start_date + timedelta(days=i)).strftime('%Y-%m-%d') for i in
                       range((e.end_date - e.start_date).days + 1)]
...
def get_quarterly_board(db: Session, year: int, quarter: int, current_user: dict):
    ...
    for p in pentesters:
        for y, w in extended_week_pairs:
            cap_matrix[p["id"]][w] = calculate_weekly_capacity(db, p["id"], y, w)
    ...
    # Base Query constraints for Maintainers
