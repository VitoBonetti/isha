...
import { X, Send, Eye, Code, CheckCircle } from 'lucide-react';
import toast from 'react-hot-toast';

interface FinalEmailModalProps {
  isOpen: boolean;
  testId: string;
  onClose: () => void;
  onSuccess: () => void;
...
export default function FinalEmailModal({ isOpen, testId, onClose, onSuccess }: FinalEmailModalProps) {
  ...
  useEffect(() => {
    if (isOpen && testId) {
      setLoading(true);
      setShowPreview(true);
      // HITTING THE NEW DRAFT ENDPOINT
      axios.get(`/api/luigi/${testId}/draft-final-email`)
        .then(res => {
          setTo(res.data.to);
          setCc(res.data.cc);
        ...
        .catch((err) => {
          toast.error(err.response?.data?.detail || "Failed to load final email draft");
          onClose(); // Close if template is missing so they don't see a blank modal
        })
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    setSending(true);
    try {
      // HITTING THE NEW SEND ENDPOINT
      await axios.post(`/api/luigi/${testId}/send-final-email`, {
        to, cc, subject, body
      });
      toast.success("Final email sent successfully!");
      onSuccess();
      onClose();
    } catch (error: any) {
      ...
      <div className="bg-white dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 rounded-2xl shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-5 border-b border-slate-100 dark:border-zinc-800 bg-slate-50/50 dark:bg-zinc-900/50 flex justify-between items-center shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-black text-lg text-slate-900 dark:text-zinc-100 leading-tight">Send Final Email</h2>
              <p className="text-xs font-medium text-slate-500">Wrap up the engagement and send final deliverables</p>
            </div>
          </div>
        ...
        {loading ? (
          <div className="flex-1 p-10 flex items-center justify-center text-slate-400 font-bold text-sm animate-pulse">
            Drafting final email from template...
          </div>
        ) : (
          <form onSubmit={handleSend} className="flex flex-col flex-1 overflow-hidden">
            <div className="p-5 overflow-y-auto flex-1 bg-slate-50/30 dark:bg-zinc-950/30 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input type="text" value={to} onChange={(e) => setTo(e.target.value)} className="w-full px-3 py-2 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-lg text-sm text-slate-900 dark:text-zinc-100 outline-none focus:border-blue-500 transition-colors" />
                </div>
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-500 uppercase">CC</label>
                  <input type="text" value={cc} onChange={(e) => setCc(e.target.value)} className="w-full px-3 py-2 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-lg text-sm text-slate-900 dark:text-zinc-100 outline-none focus:border-blue-500 transition-colors" />
                </div>
              </div>
              <div className="space-y-1">
              ...
              <div className="space-y-1 flex-1 flex flex-col min-h-[300px]">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex bg-slate-100 dark:bg-zinc-800 rounded-lg p-0.5">
                    <button type="button" onClick={() => setShowPreview(true)} className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold rounded-md transition-all ${showPreview ? 'bg-white dark:bg-zinc-700 text-emerald-600 dark:text-emerald-400 shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-zinc-300'}`}>
                      <Eye size={14} /> Preview
                    </button>
                    <button type="button" onClick={() => setShowPreview(false)} className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold rounded-md transition-all ${!showPreview ? 'bg-white dark:bg-zinc-700 text-emerald-600 dark:text-emerald-400 shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-zinc-300'}`}>
                      <Code size={14} /> HTML
                    </button>
                  </div>
                </div>
            ...
            <div className="p-4 border-t border-slate-100 dark:border-zinc-800 bg-white dark:bg-zinc-950 flex justify-end gap-3 shrink-0">
              ...
              <button type="submit" disabled={sending} className="px-5 py-2 flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 disabled:bg-emerald-400 text-white font-bold text-sm rounded-xl transition-colors">
                {sending ? (
                  <div className="w-4 h-4 border-2 border-white/20 border-t-white rounded-full animate-spin" />
                ) : (
                  <Send size={16} />
                )}
                {sending ? 'Sending...' : 'Send Final Email'}
              </button>
            </div>
          </form>
        )}
