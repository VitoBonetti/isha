    ...
    <DragDropContext onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
      <div className="flex flex-col h-screen bg-slate-50 dark:bg-zinc-950 text-sm text-slate-700 dark:text-zinc-300 overflow-hidden font-sans">
        ...
        <div className="flex justify-between items-center px-6 py-3 bg-white dark:bg-zinc-900 border-b border-slate-200 dark:border-zinc-800 z-20 shadow-sm shrink-0 pt-32">
          ...
          <div className="flex items-center gap-4">
            <div className="relative">
              ...
              <input type="text" placeholder="Search app or person..." className="pl-9 pr-4 py-1.5 bg-white dark:bg-zinc-950 border border-slate-300 dark:border-zinc-700 rounded-full text-xs w-64 focus:ring-2 focus:ring-blue-500 outline-none transition-shadow text-slate-900 dark:text-zinc-100" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
            </div>
        ...
        <div className="flex flex-1 overflow-hidden w-full relative">
          ...
          {/* Main Drag & Drop Grid */}
          <div className="flex-1 min-w-0 overflow-auto p-6 bg-slate-50 dark:bg-zinc-950/50 transition-all">
            <table className="w-max min-w-full border-collapse bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-xl shadow-sm overflow-hidden">
              <thead className="bg-slate-50 dark:bg-zinc-900">
                <tr>
                  <th className="p-4 border-b-2 border-slate-200 dark:border-zinc-800 text-left sticky top-0 left-0 z-20 bg-slate-50 dark:bg-zinc-900 shadow-[2px_0_5px_rgba(0,0,0,0.05)] min-w-[200px]">
              ...
              <tbody>
                {boardData.services.map(service => {
                  return (
                    <tr key={service.id}>
                      <td className="p-4 font-bold text-slate-900 dark:text-zinc-100 border-b border-slate-200 dark:border-zinc-800 sticky left-0 z-[5] bg-white dark:bg-zinc-900 shadow-[2px_0_5px_rgba(0,0,0,0.05)]">
                        <div className="flex items-center gap-2">
                          <div className="w-3 h-3 rounded-full shadow-sm" style={{ backgroundColor: service.theme_color }}></div>
                          {service.name}
                        </div>
                      </td>
                      {displayWeeks.map(week => {
                        const cellId = `${service.id}_${week}`;
                        // Multi-week intersection logic
                        ...
                        return (
                          <td key={cellId} className={`p-2 border-b border-r border-slate-200 dark:border-zinc-800 align-top min-h-[120px] transition-colors ${isInvalidDropTarget ? 'bg-slate-100/50 dark:bg-zinc-950/80' : isValidDropTarget ? 'bg-blue-50/50 dark:bg-blue-900/10' : ''}`}>
                              ...
                              {(provided, snapshot) => (
                                <div ref={provided.innerRef} {...provided.droppableProps} className={`min-h-[120px] w-full h-full rounded-lg p-1 ${snapshot.isDraggingOver ? 'bg-white/50 dark:bg-zinc-800/50 ring-2 ring-blue-400' : ''}`}>
                                  ...
                                  {testsInThisCell.map((test, index) => {
                                    ...
                                    // 1. Real Draggable Card (Start Week)
                                    ...
                                    // 2. Translucent Ghost Card (Spanning Weeks)
                                    else {
                                      return (
                        ...
                        {sortedBacklog.map((test, index) => {
                          ...
                          return (
                            <Draggable key={test.id} draggableId={test.id} index={index} isDragDisabled={['pentester', 'read_only'].includes(currentUser?.role)}>
                              {(provided) => (
                                <div ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps}
                                  className={`p-3 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-700 rounded-xl mb-3 cursor-grab flex justify-between items-center shadow-sm hover:shadow-md transition-all ${testMatchesSearch ? 'opacity-100' : 'opacity-30 grayscale'}`}
                                  style={{ ...provided.draggableProps.style, borderLeftWidth: '4px', borderLeftColor: service?.theme_color || '#cbd5e1' }}
                                >
