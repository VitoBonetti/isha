...
import {
  GitMerge, RefreshCw, Check, Search, Filter, ShieldCheck,
  Building2, ExternalLink, Wand2, AlertCircle, ChevronLeft,
...
export default function AssetReconciliationView() {
  ...
  // NEW: Track if a scan has been performed yet
  ...
  // Track selected candidate for each Mario asset ID: { [mario_raw_asset_id]: selected_kiss24_uuid }
  ...
  const toggleAllAssets = () => {
    if (selectedAssets.size === paginatedCandidates.length) {
      setSelectedAssets(new Set()); // Deselect all on current page
    } else {
      // Select all ON CURRENT PAGE that have a valid top suggestion
      const newSet = new Set(selectedAssets);
  ...
  const handleBulkApprove = async () => {
    ...
    // Prepare the payload using the currently selected dropdown values
    ...
    }).filter(a => a.kiss24_uuid); // Ensure they actually have a selection
    ...
    try {
      ...
      // Optimistically remove all successfully linked assets from the view
  ...
  const loadCandidates = async () => {
    setLoading(true);
    try {
      // Limit=0 pulls all candidates from backend
      const res = await axios.get('/api/kiss24/reconciliation-candidates?limit=0');
      setCandidates(res.data);

      // Pre-select the top suggestion (#1 match) for each asset automatically
  ...
  // Filter options for Country dropdown
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-zinc-950 text-slate-900 dark:text-zinc-100">
      <TopNav />
      <div className="pt-28 pb-12 px-6 w-full max-w-[1600px] mx-auto">

        {/* Toolbar Header */}
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
          <div>
            <button onClick={() => navigate('/raw')} className="flex items-center gap-1 text-xs font-bold text-slate-400 hover:text-slate-700 dark:hover:text-zinc-200 mb-2 transition-colors cursor-pointer">
              <ChevronLeft size={14} /> Back to Raw Assets
            </button>
                          ...
                          <td className="p-4">
                            {candidate.top_suggestions.length > 0 ? (
                              <div className="flex items-center gap-2">
                                ...
                                {/* DYNAMIC KEEP SECURE 24 ASSET LINK */}
                                {selectedUuid && (
                                  <a
