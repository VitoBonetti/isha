...
import { Search, Upload, Plus, Filter, ChevronDown } from "lucide-react";
...
export default function RawAssetsView() {
  ...
  const handlePromoteSelected = async () => {
    ...
    try {
      await axios.post("/api/assets/promote", { 
        raw_asset_ids: selectedAssets 
      });
      setSelectedAssets([]);
      fetchRawAssets(); // Refresh the list
      alert(`Successfully promoted ${selectedAssets.length} assets to pool!`);
    } catch (error) {
      console.error("Failed to promote assets:", error);
      alert("Failed to promote assets");
    }
  };
      ...
      <div className="pt-24 pb-8 px-6 max-w-7xl mx-auto">
        ...
        <div className="flex flex-wrap gap-4 items-center justify-between bg-white dark:bg-zinc-900 p-4 rounded-xl border border-slate-200 dark:border-zinc-800 shadow-sm">
          ...
          <div className="flex items-center gap-3">
            ...
            <button className="flex items-center gap-2 px-4 py-2 bg-emerald-500 text-white rounded-lg hover:bg-emerald-600 transition-colors font-medium">
                  ...
                  {assets.map((asset) => (
                    <tr 
                      ...
                      className={`hover:bg-slate-50 dark:hover:bg-zinc-800/30 transition-colors ${
                        selectedAssets.includes(asset.id) ? 'bg-emerald-50 dark:bg-emerald-900/20' : ''
                      }`}
                    >
                      <td className="px-6 py-4">
                        <input
                          ...
                          className="h-4 w-4 text-emerald-500 rounded border-slate-300 focus:ring-emerald-500"
                        />
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                        <div className="text-sm">{asset.service_name || 'Unassigned'}</div>
                      </td>
                      <td className="px-6 py-4 text-right">
                        {asset.is_promoted ? (
                          ...
                          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-slate-100 dark:bg-zinc-700/30 text-slate-600 dark:text-zinc-400">
                            Raw
                          </span>
                        )}
                      </td>
              ...
              {assets.length > 0 && (
                <div className="px-6 py-4 border-t border-slate-200 dark:border-zinc-700 flex items-center justify-between">
                  <p className="text-sm text-slate-500">
                    Showing page {page} of assets
                  </p>
                  <div className="flex gap-2">
                    <button
                      onClick={() => setPage(p => Math.max(1, p - 1))}
                      disabled={page === 1}
                      className="px-3 py-1 rounded-lg border border-slate-300 dark:border-zinc-700 hover:bg-slate-100 dark:hover:bg-zinc-800 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      Previous
                    </button>
                    <button
                      onClick={() => setPage(p => p + 1)}
                      className="px-3 py-1 rounded-lg border border-slate-300 dark:border-zinc-700 hover:bg-slate-100 dark:hover:bg-zinc-800"
                    >
                      Next
                    </button>
                  </div>
