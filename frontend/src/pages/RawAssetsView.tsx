...
export default function RawAssetsView() {
  ...
  const handlePromoteSingle = async (assetId: string) => {
    const toastId = toast.loading("Promoting asset...");
    try {
      await axios.post("/api/assets/promote", { raw_asset_ids: [assetId] });
      toast.dismiss(toastId);
      toast.success("Asset promoted to Active Pool!");
      fetchRawAssets();
    } catch (error) {
      toast.dismiss(toastId);
